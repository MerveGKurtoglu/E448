...
{
    //std::cout << "Microenvironment " << environment[0] << std::endl;
    
    double number_of_pixels = environment.shape[0] * environment.shape[1] ;
    //std::cout << "Microenvironment shape : " << number_of_pixels << std::endl;
    
    //creating environment data
    double env_data [environment.shape[0]][environment.shape[1]];
    //std::cout << env_data[0][0] << std::endl;

    for( int i=0 ; i < environment.shape[0] ; i++ )
	{ 
        for( int j=0 ; j < environment.shape[1] ; j++ )
        {
            int index = environment.shape[1] * j + i;
            std::cout << environment[index] << std::endl;
            double value = environment[index];
            env_data[i,j] = value;
        }
    } 
    
    //std::cout << "Matrix = " << env_data[0][0] << std::endl;
}
